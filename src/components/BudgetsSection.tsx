...
export function BudgetsSection() {
    ...
    const handleDelete = async (id: string) => {
        ...
        load()
        addToast('warning', 'Orçamento excluído')
    }
    ...
    return (
        <div className="bg-surface border border-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                    {overBudget > 0 && (
                        <span className="text-[10px] bg-danger-dim text-danger border border-danger/30 px-2 py-0.5 rounded-full">
            ...
            {loading ? (
                ...
                <div className="space-y-2">
                    {/* Cabeçalho */}
                    <div className="grid grid-cols-12 gap-2 px-3 py-1.5 text-[10px] text-muted uppercase tracking-wider font-semibold">
                    ...
                    {budgets.map((b, i) => {
                        ...
                        return (
                            <div key={b.id} className="grid grid-cols-12 gap-2 items-center px-3 py-2.5 rounded-xl bg-card border border-border hover:border-[#2a3a52] transition-all">
                                ...
                                <div className={`col-span-2 text-right text-sm font-mono ${isOver ? 'text-danger' : 'text-subtle'}`}>
                                    {fmt(b.spent || 0)}
                                </div>
                                <div className="col-span-4 space-y-0.5">
                                    ...
                                    <div className="flex justify-between text-[10px]">
                                        ...
                                        {isOver && <span className="text-danger">Excedeu {fmt(Number(b.spent) - Number(b.amount))}</span>}
                                    </div>
                                </div>
                                <div className="col-span-1 flex gap-1 justify-end">
                                    <button onClick={() => { setEditing(b); setShowModal(true) }} className="p-1 rounded hover:bg-border text-muted hover:text-info transition-all">
                                        <Pencil className="w-3 h-3" />
                                    </button>
                                    <button onClick={() => handleDelete(b.id!)} className="p-1 rounded hover:bg-border text-muted hover:text-danger transition-all">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
            <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    <div>
                        <label className="text-[10px] text-muted uppercase tracking-wider font-semibold">Categoria</label>
                    ...
                    <div>
                        <label className="text-[10px] text-muted uppercase tracking-wider font-semibold">Valor Mensal</label>
                        <input type="number" step="0.01" min="0" value={amount} onChange={e => setAmount(e.target.value)}
                            className="w-full bg-surface border border-border rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-accent/50 transition-colors" />
                    </div>
                    <div>
                        <label className="text-[10px] text-muted uppercase tracking-wider font-semibold">Período</label>
