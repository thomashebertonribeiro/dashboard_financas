...
export function CreditCardsSection() {
    ...
    const handleDelete = async (id: string) => {
        ...
        load()
        addToast('warning', 'Cartão excluído')
    }
        ...
        <div className="bg-surface border border-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    ...
                    {totalLimits > 0 && (
                        <span className="text-xs text-muted font-mono">Limite total: {fmt(totalLimits)}</span>
                    )}
                </div>
                <button
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    {cards.map(card => {
                        ...
                        return (
                            <div key={card.id} className="bg-card border border-border rounded-2xl p-4 flex flex-col gap-3">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-muted text-[10px]">{BRAND_LABELS[card.brand || 'other']}</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-1">
                                        <button onClick={() => { setEditing(card); setShowModal(true) }} className="p-1.5 rounded-lg hover:bg-border text-muted hover:text-info transition-all">
                                            <Pencil className="w-3.5 h-3.5" />
                                        </button>
                                        <button onClick={() => handleDelete(card.id!)} className="p-1.5 rounded-lg hover:bg-border text-muted hover:text-danger transition-all">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div className="bg-surface rounded-xl p-2.5">
                                        <p className="text-muted text-[10px]">Fechamento</p>
                                        <p className="text-white font-mono font-semibold">Dia {card.closing_day}</p>
                                    </div>
                                    <div className="bg-surface rounded-xl p-2.5">
                                        <p className="text-muted text-[10px]">Vencimento</p>
                                ...
                                {card.limit_amount && card.limit_amount > 0 && (
                                    <div className="space-y-1">
                                        <div className="flex justify-between text-xs">
                                            <span className="text-subtle">Limite</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
            <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    ...
                    <div>
                        <label className="text-[10px] text-muted uppercase tracking-wider font-semibold">Bandeira</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-[10px] text-muted uppercase tracking-wider font-semibold">Dia Fechamento</label>
                            <input type="number" min="1" max="31" value={closingDay} onChange={e => setClosingDay(e.target.value)}
                                className="w-full bg-surface border border-border rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-accent/50 transition-colors" />
                        </div>
                        <div>
                            <label className="text-[10px] text-muted uppercase tracking-wider font-semibold">Dia Vencimento</label>
                            <input type="number" min="1" max="31" value={dueDay} onChange={e => setDueDay(e.target.value)}
                                className="w-full bg-surface border border-border rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-accent/50 transition-colors" />
                        </div>
                    </div>
                    <div>
                        <label className="text-[10px] text-muted uppercase tracking-wider font-semibold">Limite (opcional)</label>
